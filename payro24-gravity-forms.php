...
/**
 * Plugin Name: payro24 gateway - Gravity Forms
 * Author: payro24
 * Description: <a href="https://payro24.ir">payro24</a> secure payment gateway for Gravity Forms.
 * Version: 1.0.4
 * Author URI: https://payro24.ir
 * Author Email: info@payro24.ir
 * Text Domain: payro24-gravity-forms
 */

if (!defined('ABSPATH')) exit;

register_activation_hook(__FILE__, array('GF_Gateway_payro24', "add_permissions"));
add_action('init', array('GF_Gateway_payro24', 'init'));

require_once('lib/payro24_DB.php');
require_once('lib/payro24_Chart.php');

class GF_Gateway_payro24
{
    public static $author = "payro24";
    ...
    public static function init()
    {
        ...
        if (is_admin() && self::has_access()) {
            ...
            if (get_option("gf_payro24_configured")) {
                ...
                add_action('gform_form_settings_page_payro24', array(__CLASS__, 'feed_page'));
            }

            if (rgget("page") == "gf_settings") {
                RGForms::add_settings_page(array(
                        'name' => 'gf_payro24',
                        'tab_label' => __('درگاه payro24', 'gravityformspayro24'),
                        'title' => __('تنظیمات درگاه payro24', 'gravityformspayro24'),
                        'handler' => array(__CLASS__, 'settings_page'),
                    )
                );
            }

            if (self::is_payro24_page()) {
                wp_enqueue_script(array("sack"));
                self::setup();
            }

            add_action('wp_ajax_gf_payro24_update_feed_active', array(__CLASS__, 'update_feed_active'));
        }
        if (get_option("gf_payro24_configured")) {
        ...
        add_filter('gf_payment_gateways', array(__CLASS__, 'gravityformspayro24'), 2);
        do_action('gravityforms_gateways');
        do_action('gravityforms_payro24');
    ...
    public static function alter_submit_button($button_input, $form){
        ...
        if ($has_product && !empty($config)) {
            $button_input .= sprintf(
                '<div id="payro24-pay-id-%1$s" class="payro24-logo" style="font-size: 14px;padding: 5px 0;"><img src="%2$s" style="display: inline-block;vertical-align: middle;width: 70px;">%3$s</div>',
                $form['id'], plugins_url( '/assets/logo.svg', __FILE__ ), __( 'پرداخت امن با پیرو', 'gravityformspayro24' )
            );
            $button_input .=
                "<script>
                gform.addAction('gform_post_conditional_logic_field_action', function (formId, action, targetId, defaultValues, isInit) {
                    gf_do_action(action, '#payro24-pay-id-'+ formId, true, defaultValues, isInit, null, formId);
                });
            </script>";
        }
    ...
    protected static function has_access($required_permission = 'gravityforms_payro24')
    {
    ...
    private static function is_payro24_page()
    {

        $current_page = in_array(trim(rgget("page")), array('gf_payro24', 'payro24'));
        $current_view = in_array(trim(rgget("view")), array('gf_payro24', 'payro24'));
        $current_subview = in_array(trim(rgget("subview")), array('gf_payro24', 'payro24'));
        return $current_page || $current_view || $current_subview;
    }

    private static function setup()
    {
        if (get_option("gf_payro24_version") != self::$version) {
            payro24_DB::update_table();
            update_option("gf_payro24_version", self::$version);
        }
    }
    ...
    public static function admin_notice_persian_gf()
    {
        ...
        $message = sprintf(__("برای استفاده از نسخه جدید درگاه پرداخت پیرو برای گرویتی فرم، نصب بسته فارسی ساز نسخه 2.3.1 به بالا الزامی است. برای نصب فارسی ساز %sکلیک کنید%s.", "gravityformspayro24"), '<a href="' . admin_url("plugin-install.php?tab=plugin-information&plugin=persian-gravity-forms&TB_iframe=true&width=772&height=884") . '">', '</a>');
    ...
    {
        $class = 'notice notice-error';
        $message = sprintf(__("درگاه payro24 نیاز به گرویتی فرم نسخه %s به بالا دارد. برای بروز رسانی هسته گرویتی فرم به %sسایت گرویتی فرم فارسی%s مراجعه نمایید .", "gravityformspayro24"), self::$min_gravityforms_version, "<a href='http://gravityforms.ir/11378' target='_blank'>", "</a>");
        printf('<div class="%1$s"><p>%2$s</p></div>', $class, $message);
    }

    public static function gravityformspayro24($form, $entry)
    {
        $payro24 = array(
            'class' => (__CLASS__ . '|' . self::$author),
            'title' => __('payro24', 'gravityformspayro24'),
            'param' => array(
                'email' => __('ایمیل', 'gravityformspayro24'),
                'mobile' => __('موبایل', 'gravityformspayro24'),
                'desc' => __('توضیحات', 'gravityformspayro24')
            )
        );

        return apply_filters(self::$author . '_gf_payro24_detail', apply_filters(self::$author . '_gf_gateway_detail', $payro24, $form, $entry), $form, $entry);
    }
    ...
    public static function add_permissions()
    {
        ...
        foreach ((array)$editable_roles as $role => $details) {
            if ($role == 'administrator' || in_array('gravityforms_edit_forms', $details['capabilities'])) {
                $wp_roles->add_cap($role, 'gravityforms_payro24');
                $wp_roles->add_cap($role, 'gravityforms_payro24_uninstall');
            }
        }
    }
    ...
    {
        return array_merge($caps, array("gravityforms_payro24", "gravityforms_payro24_uninstall"));
    }

    public static function tooltips($tooltips)
    {
        $tooltips["gateway_name"] = __("تذکر مهم : این قسمت برای نمایش به بازدید کننده می باشد و لطفا جهت جلوگیری از مشکل و تداخل آن را فقط یکبار تنظیم نمایید و از تنظیم مکرر آن خود داری نمایید .", "gravityformspayro24");
    ...
    {
        $permission = "gravityforms_payro24";
        if (!empty($permission)) {
            $menus[] = array(
                "name" => "gf_payro24",
                "label" => __("payro24", "gravityformspayro24"),
                "callback" => array(__CLASS__, "payro24_page"),
    ...
    {
        $plugins[basename(dirname(__FILE__))] = "payro24";
    ...
    public static function feed_page()
    {
        ...
        <h3>
			<span><i class="fa fa-credit-card"></i> <?php esc_html_e('payro24', 'gravityformspayro24') ?>
                <a id="add-new-confirmation" class="add-new-h2"
                   href="<?php echo esc_url(admin_url('admin.php?page=gf_payro24&view=edit&fid=' . absint(rgget("id")))) ?>"><?php esc_html_e('افزودن فید جدید', 'gravityformspayro24') ?></a></span>
            <a class="add-new-h2"
               href="admin.php?page=gf_payro24&view=stats&id=<?php echo absint(rgget("id")) ?>"><?php _e("نمودار ها", "gravityformspayro24") ?></a>
        </h3>
    ...
    public static function get_active_config($form)
    {
        ...
        $configs = payro24_DB::get_feed_by_form($form["id"], true);

        $configs = apply_filters(self::$author . '_gf_payro24_get_active_configs', apply_filters(self::$author . '_gf_gateway_get_active_configs', $configs, $form), $form);
        ...
        if (!empty($configs) && is_array($configs)) {
            ...
            foreach ($configs as $config) {
                if (self::has_payro24_condition($form, $config)) {
                    $return = $config;
                }
                break;
            }
        ...
        self::$config = apply_filters(self::$author . '_gf_payro24_get_active_config', apply_filters(self::$author . '_gf_gateway_get_active_config', $return, $form), $form);

        return self::$config;
    }

    public static function has_payro24_condition($form, $config)
    {
        ...
        if (empty($config['meta']['payro24_conditional_enabled'])) {
            return true;
        }
        ...
        $type = !empty($config['meta']['payro24_conditional_type']) ? strtolower($config['meta']['payro24_conditional_type']) : '';
    ...
    public static function payro24_page()
    {
        $view = rgget("view");
        if ($view == "edit") {
            require_once( self::get_base_path() . '/templates/config_page.php' );
        } else if ($view == "stats") {
            payro24_Chart::stats_page();
        } else {
            self::list_page('');
        }
    }
    ...
    {
        check_ajax_referer('gf_payro24_update_feed_active', 'gf_payro24_update_feed_active');
        $id = absint(rgpost('feed_id'));
        $feed = payro24_DB::get_feed($id);
        payro24_DB::update_feed($id, $feed["form_id"], sanitize_text_field(rgpost( "is_active" )), $feed["meta"]);
    }
    ...
    public static function payment_entry_detail($form_id, $entry)
    {
        ...
        if (!empty($payment_gateway) && $payment_gateway == "payro24") {
            ...
            <hr/>
            <strong>
                <?php _e('اطلاعات تراکنش :', 'gravityformspayro24') ?>
            </strong>
            <br/>
            <br/>
            <?php
            ...
            if ($payment_status == 'Paid') {
                $payment_status_persian = __('موفق', 'gravityformspayro24');
            }
            ...
            if ($payment_status == 'Cancelled') {
                $payment_status_persian = __('منصرف شده', 'gravityformspayro24');
            }

            if ($payment_status == 'Failed') {
                $payment_status_persian = __('ناموفق', 'gravityformspayro24');
            }

            if ($payment_status == 'Processing') {
                $payment_status_persian = __('معلق', 'gravityformspayro24');
            }

            if (!strtolower(rgpost("save")) || RGForms::post("screen_mode") != "edit") {
                echo __('وضعیت پرداخت : ', 'gravityformspayro24') . $payment_status_persian . '<br/><br/>';
                echo __('تاریخ پرداخت : ', 'gravityformspayro24') . '<span style="">' . $payment_date . '</span><br/><br/>';
                echo __('مبلغ پرداختی : ', 'gravityformspayro24') . GFCommon::to_money($payment_amount, rgar($entry, "currency")) . '<br/><br/>';
                echo __('کد رهگیری : ', 'gravityformspayro24') . $transaction_id . '<br/><br/>';
                echo __('درگاه پرداخت : payro24', 'gravityformspayro24');
            } else {
                $payment_string = '';
                ...
                if ($transaction_type == 1) {
                    if ($payment_status != "Paid") {
                        $payment_string .= '<option value="Paid">' . __('موفق', 'gravityformspayro24') . '</option>';
                    }
                }

                if ($transaction_type == 2) {
                    if ($payment_status != "Active") {
                        $payment_string .= '<option value="Active">' . __('موفق', 'gravityformspayro24') . '</option>';
                    }
                }
                ...
                if ($payment_status != "Failed") {
                    $payment_string .= '<option value="Failed">' . __('ناموفق', 'gravityformspayro24') . '</option>';
                }

                if ($payment_status != "Cancelled") {
                    $payment_string .= '<option value="Cancelled">' . __('منصرف شده', 'gravityformspayro24') . '</option>';
                }

                if ($payment_status != "Processing") {
                    $payment_string .= '<option value="Processing">' . __('معلق', 'gravityformspayro24') . '</option>';
                }

                $payment_string .= '</select>';

                echo __('وضعیت پرداخت :', 'gravityformspayro24') . $payment_string . '<br/><br/>';
                ?>
                <div id="edit_payment_status_details" style="display:block">
                    <table>
                        <tr>
                            <td><?php _e('تاریخ پرداخت :', 'gravityformspayro24') ?></td>
                            <td><input type="text" id="payment_date" name="payment_date"
                                       value="<?php echo $payment_date ?>"></td>
                        </tr>
                        <tr>
                            <td><?php _e('مبلغ پرداخت :', 'gravityformspayro24') ?></td>
                            <td><input type="text" id="payment_amount" name="payment_amount"
                                       value="<?php echo $payment_amount ?>"></td>
                        </tr>
                        <tr>
                            <td><?php _e('شماره تراکنش :', 'gravityformspayro24') ?></td>
                            <td><input type="text" id="payro24_transaction_id" name="payro24_transaction_id"
                ...
                <?php
                echo __('درگاه پرداخت : payro24 (غیر قابل ویرایش)', 'gravityformspayro24');
            }
    ...
    public static function get_order_total($form, $entry)
    {
        ...
        return apply_filters(self::$author . '_payro24_get_order_total', apply_filters(self::$author . '_gateway_get_order_total', $total, $form, $entry), $form, $entry);
    }
    ...
    public static function update_payment_entry($form, $entry_id)
    {
        ...
        $payment_transaction    = sanitize_text_field( rgpost("payro24_transaction_id") );
        ...
        global $current_user;
        $user_id = 0;
        $user_name = __("مهمان", 'gravityformspayro24');
        ...
        switch (rgar($entry, "payment_status")) {
            case "Active" :
                $new_status = __('موفق', 'gravityformspayro24');
                break;
            ...
            case "Cancelled" :
                $new_status = __('منصرف شده', 'gravityformspayro24');
                break;

            case "Failed" :
                $new_status = __('ناموفق', 'gravityformspayro24');
                break;

            case "Processing" :
                $new_status = __('معلق', 'gravityformspayro24');
                break;
        }

        RGFormsModel::add_note($entry["id"], $user_id, $user_name, sprintf(__("اطلاعات تراکنش به صورت دستی ویرایش شد . وضعیت : %s - مبلغ : %s - کد رهگیری : %s - تاریخ : %s", "gravityformspayro24"), $new_status, GFCommon::to_money($entry["payment_amount"], $entry["currency"]), $payment_transaction, $entry["payment_date"]));
    ...
    public static function uninstall()
    {
        if (!self::has_access("gravityforms_payro24_uninstall")) {
            die(__("شما مجوز کافی برای این کار را ندارید . سطح دسترسی شما پایین تر از حد مجاز است . ", "gravityformspayro24"));
        }
        payro24_DB::drop_tables();
        delete_option("gf_payro24_settings");
        delete_option("gf_payro24_configured");
        delete_option("gf_payro24_version");
    ...
    public static function Request($confirmation, $form, $entry, $ajax)
    {
        ...
        do_action('gf_payro24_request_1', $confirmation, $form, $entry, $ajax);

        if (apply_filters('gf_payro24_request_return', apply_filters('gf_gateway_request_return', false, $confirmation, $form, $entry, $ajax), $confirmation, $form, $entry, $ajax)) {
            return $confirmation;
        }
        $entry_id = $entry['id'];
        ...
        if ($confirmation != 'custom') {
            ...
            gform_update_meta($entry_id, 'payro24_feed_id', $config['id']);
            ...
            $Amount = apply_filters(self::$author . "_gform_form_payro24_price_{$form['id']}", apply_filters(self::$author . "_gform_form_payro24_price", $Amount, $form, $entry), $form, $entry);
            ...
            $Amount = apply_filters(self::$author . "_gform_payro24_price_{$form['id']}", apply_filters(self::$author . "_gform_payro24_price", $Amount, $form, $entry), $form, $entry);
            ...
            $Amount = gform_get_meta(rgar($entry, 'id'), 'payro24_part_price_' . $form['id']);
            ...
            $Amount = apply_filters(self::$author . "_gform_custom_payro24_price_{$form['id']}", apply_filters(self::$author . "_gform_custom_payro24_price", $Amount, $form, $entry), $form, $entry);
            $Amount = apply_filters(self::$author . "_gform_gateway_price_{$form['id']}", apply_filters(self::$author . "_gform_gateway_price", $Amount, $form, $entry), $form, $entry);
            $Amount = apply_filters(self::$author . "_gform_payro24_price_{$form['id']}", apply_filters(self::$author . "_gform_payro24_price", $Amount, $form, $entry), $form, $entry);

            $Description = gform_get_meta(rgar($entry, 'id'), 'payro24_part_desc_' . $form['id']);
            $Description = apply_filters(self::$author . '_gform_payro24_gateway_desc_', apply_filters(self::$author . '_gform_custom_gateway_desc_', $Description, $form, $entry), $form, $entry);

            $Name = gform_get_meta(rgar($entry, 'id'), 'payro24_part_name_' . $form['id']);
            $Mail = gform_get_meta(rgar($entry, 'id'), 'payro24_part_email_' . $form['id']);
            $Mobile = gform_get_meta(rgar($entry, 'id'), 'payro24_part_mobile_' . $form['id']);

            $entry_id = GFAPI::add_entry($entry);
            $entry = GFPersian_Payments::get_entry($entry_id);

            do_action('gf_gateway_request_add_entry', $confirmation, $form, $entry, $ajax);
            do_action('gf_payro24_request_add_entry', $confirmation, $form, $entry, $ajax);
        ...
        do_action('gf_payro24_request_2', $confirmation, $form, $entry, $ajax);

        $Amount = GFPersian_Payments::amount($Amount, 'IRR', $form, $entry);
        if (empty($Amount) || !$Amount || $Amount > 500000000 || $Amount < 1000) {
            $Message = __('مبلغ ارسالی اشتباه است.', 'gravityformspayro24');
        }
        else {
            $data = array(
                'order_id' => $entry_id,
            ...
            if ( is_wp_error( $response ) ) {
                ...
                $Message = sprintf(__('خطا هنگام ایجاد تراکنش. پیام خطا: %s', 'gravityformspayro24'), $error);
            }
        ...
        $Message = !empty($Message) ? $Message : __('خطایی رخ داده است.', 'gravityformspayro24');
        $confirmation = __('متاسفانه نمیتوانیم به درگاه متصل شویم. علت : ', 'gravityformspayro24') . $Message;
        ...
        RGFormsModel::add_note($entry_id, $user_id, $user_name, sprintf(__('خطا در اتصال به درگاه رخ داده است : %s', "gravityformspayro24"), $Message));
    ...
    public static function get_gname()
    {
        ...
        if (isset($settings["gname"])) {
            ...
            $gname = __('payro24', 'gravityformspayro24');
        }
        return $gname;
    }
    ...
    private static function Return_URL($form_id, $entry_id)
    {
        ...
        return apply_filters(self::$author . '_payro24_return_url', apply_filters(self::$author . '_gateway_return_url', $pageURL, $form_id, $entry_id, __CLASS__), $form_id, $entry_id, __CLASS__);
    }
    ...
    {
        if (apply_filters('gf_gateway_payro24_return', apply_filters('gf_gateway_verify_return', false))) {
            return;
        }
        if (!self::is_gravityforms_supported()) {
            return;
        ...
        if (is_wp_error($entry) || !empty($entry["payment_date"])
            || !(isset($entry["payment_method"]) && $entry["payment_method"] == 'payro24')) {
            return;
        }
        ...
        if ($payment_type != 'custom') {
            ...
            $config = apply_filters(self::$author . '_gf_payro24_config', apply_filters(self::$author . '_gf_gateway_config', array(), $form, $entry), $form, $entry);
        }
        ...
        if ($payment_type == 'custom') {
            $Amount = $Total = gform_get_meta($entry["id"], 'payro24_part_price_' . $form_id);
        } else {
        ...
        if ($Status == 'completed') {
            ...
            if ($transaction_type == 2) {
                ...
                RGFormsModel::add_note($entry["id"], $user_id, $user_name, __("تغییرات اطلاعات فیلدها فقط در همین پیام ورودی اعمال خواهد شد و بر روی وضعیت کاربر تاثیری نخواهد داشت .", "gravityformspayro24"));
            } else {
            ...
            if ($free == true) {
                ...
                $message = $Note = sprintf(__('وضعیت پرداخت : رایگان - بدون نیاز به درگاه پرداخت', "gravityformspayro24"));
            } else {
                $message = sprintf(__(' پرداخت شما با موفقیت انجام شد. شماره سفارش: %s - کد رهگیری: %s', "gravityformspayro24"), $result->order_id, $result->track_id);
                $Note = sprintf(__(' وضعیت تراکنش: %s - کد رهگیری: %s - شماره کارت: %s شماره کارت هش شده:%s', "gravityformspayro24"), self::getStatus($result->status), $result->track_id, $result->payment->card_no, $result->payment->hashed_card_no);
                $Note .= print_r($result, true);
            }

            GFAPI::update_entry($entry);

            if (apply_filters(self::$author . '_gf_payro24_post', apply_filters(self::$author . '_gf_gateway_post', ($payment_type != 'custom'), $form, $entry), $form, $entry)) {
            ...
            $user_registration_slug = apply_filters('gf_user_registration_slug', 'gravityformsuserregistration');
            $payro24_config = array('meta' => array());
                        ...
                        if (is_callable(array($addon, 'get_instance'))) {
                            ...
                            if (is_object($addon) && method_exists($addon, 'get_slug')) {
                                ...
                                if ($slug != $user_registration_slug) {
                                    $payro24_config['meta']['delay_' . $slug] = true;
                                }
                            }
                        }
                    }
            ...
            if (!empty($config["meta"]["type"]) && $config["meta"]["type"] == "subscription") {
                $payro24_config['meta']['delay_' . $user_registration_slug] = true;
            }

            do_action("gform_payro24_fulfillment", $entry, $config, $transaction_id, $Total);
            do_action("gform_gateway_fulfillment", $entry, $config, $transaction_id, $Total);
            do_action("gform_payro24_fulfillment", $entry, $payro24_config, $transaction_id, $Total);
        } else {
            ...
            $message = $Note = sprintf(__('وضعیت پرداخت :%s (کد خطا: %s) - مبلغ قابل پرداخت : %s', "gravityformspayro24"), self::getStatus($status_code), $status_code, $Total_Money);
        ...
        if (apply_filters(self::$author . '_gf_payro24_verify', apply_filters(self::$author . '_gf_gateway_verify', ($payment_type != 'custom'), $form, $entry), $form, $entry)) {
    ...
    {
        $feed_id = gform_get_meta($entry["id"], "payro24_feed_id");
        $feed = !empty($feed_id) ? payro24_DB::get_feed($feed_id) : '';
        $return = !empty($feed) ? $feed : false;

        return apply_filters(self::$author . '_gf_payro24_get_config_by_entry', apply_filters(self::$author . '_gf_gateway_get_config_by_entry', $return, $entry), $entry);
    }
    ...
    public static function payro24_get_message($massage, $track_id, $order_id)
    {
